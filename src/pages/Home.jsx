  ...
  const handleSubmitLogin = async (e) => {
    ...
    try {
      await api.post("/login", user).then((response) => {
        if (response.data.status) {
          ...
          setAlert({
            open: true,
            status: "Error",
            message: response?.data?.message || "Algo salió mal",
          });
        }
        clearTimeout(timeoutRef.current);
        clearInterval(intervalRef.current);
      ...
      setAlert({
        open: true,
        status: "Error",
        // message: error.erros.data?.Message || "Algo salió mal",
      });
    }
          ...
          >
            <div>
              ...
              <p className="text-center  font-yesevaOne text-lg uppercase ">
                Inicia sesión como personal de {submitStatus.toLowerCase()}
              </p>
            ...
            <Button3D
            className="mt-3"
              color={submitStatus == "Secretaria" ? 'red' : 'blue1'}
              text='Entrar'
              fClick={(e) => {
